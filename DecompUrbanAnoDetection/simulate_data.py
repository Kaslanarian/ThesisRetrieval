...
def generate_ny_flow(weight=2):
    feature = sio.loadmat(
        "data/feature.mat"
    )["feature"]  # (13176, 82, 52)->(N, region, hour+weekday+weather+geo_embedding)
    flow = sio.loadmat(
        "data/flow.mat"
    )["flow"]  # (13176, 82, 4)->(N, region, [taxi_in, taxi_out, bike_in, bike_out])
    week_num = 1
    repeat_num = 20
    # weather*5;weekday*7;hour*24
    normal_x = feature[:week_num * 7 * 24, :, :]  # (168, 82, 52)
    normal_y = np.zeros_like(flow[:week_num * 7 * 24, :, :])
    for i in range(50):
        normal_y += flow[i * week_num * 7 * 24:(i + 1) * week_num * 7 *
                         24, :, :]
    ...
    for i in tqdm(range(12), desc="Geo embedding"):
    ...
    for hour in rnd_hours:
        ...
        for area in rnd_areas:
            scale = weight
            normal_y[hour:hour + last_hour, area:area +
                     effect_area, :] = scale * normal_y[hour:hour + last_hour,
                                                        area:area +
                                                        effect_area, :]
    ...
    rnd_hours = np.random.randint(0,
                                  n - test_week_num * 7 * 24,
                                  size=num_train_ano)
    for hour in rnd_hours:
        rnd_areas = np.random.randint(0, m - effect_area, size=2)
        for area in rnd_areas:
    ...
    sio.savemat("data/fake_data.mat", {
        "x": normal_x,
        "y": normal_y,
        "index": indexs,
        "label": labels
