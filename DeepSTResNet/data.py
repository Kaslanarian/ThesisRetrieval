...
from minmax_scale import MinMaxNormalization
...
def stat(fname):
    def get_nb_timeslot(f):
        ...
        ts_str, te_str = time.strftime("%Y-%m-%d",
                                       ts), time.strftime("%Y-%m-%d", te)
...
def load_data(T=48,
    ...
    whole_data = np.vstack(copy(data_all))
    data_train, data_test = train_test_split(whole_data,
                                             test_size=test_size,
                                             shuffle=False)
    # data_train = np.vstack(copy(data_all))[:-len_test]
    print('train_data shape: ', data_train.shape)
    mmn = MinMaxNormalization(feature_range=(-1, 1))
    mmn.fit(data_train)
    ...
    XT_train, XT_test = train_test_split(
        XT,
        test_size=test_size,
        shuffle=False,
    )
    ...
    if metadata_dim is not None:
        meta_feature_train, meta_feature_test = train_test_split(
            meta_feature, test_size=test_size, shuffle=False)
